...
class ElectronicWidget extends ConsumerStatefulWidget {
  const ElectronicWidget({super.key});

  @override
  ConsumerState<ElectronicWidget> createState() => _ProductsListWidgetState();
}

class _ProductsListWidgetState extends ConsumerState<ElectronicWidget>
    with AutomaticKeepAliveClientMixin {
  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      final notifier = ref.read(homeProvider.notifier);
      notifier.getElectronicCategory();
    });
    super.initState();
  }
    ...
    return CustomScrollView(
      slivers: [
        state.isProductLoading
            ...
            : state.electronicCategory?.data?.length != 0 &&
                    state.electronicCategory?.data != null
                ? SliverGrid(
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                      crossAxisCount: 2, // Number of columns
                      crossAxisSpacing: 10.0.h, // Space between columns
