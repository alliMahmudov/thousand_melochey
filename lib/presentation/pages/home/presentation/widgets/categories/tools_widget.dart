...
class ToolsWidget extends ConsumerStatefulWidget {
  const ToolsWidget({super.key});

  @override
  ConsumerState<ToolsWidget> createState() => _ProductsListWidgetState();
}

class _ProductsListWidgetState extends ConsumerState<ToolsWidget>
    with AutomaticKeepAliveClientMixin {
  @override
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      final notifier = ref.read(homeProvider.notifier);
      notifier.getToolsCategory();
    });
    super.initState();
  }
    ...
    return CustomScrollView(
      slivers: [
        state.isProductLoading
            ...
            : state.toolsCategory?.data?.length != 0 &&
                    state.toolsCategory?.data != null
                ? SliverGrid(
                    gridDelegate: SliverGridDelegateWithFixedCrossAxisCount(
                      crossAxisCount: 2, // Number of columns
                      crossAxisSpacing: 10.0.h, // Space between columns
